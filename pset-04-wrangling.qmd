...
cases |>
  ...
  mutate(
    year = year(date),
    month = month(date, label = TRUE, abbr = FALSE, locale = "en_US.UTF-8")
  ) |>
  group_by(year, month) |>
...
deaths |>
  ...
  geom_col(fill = "red") +
  labs(title = "Top 10 States by Total COVID-19 Deaths",x = "Total Deaths",y = "State"
  ) +
  theme_minimal()
```